...
void* Reader(void* arg) {
    ...
    while (true) {
        ...
        // инфа о пакостях ридера в консоль и в файл
        std::cout << "Reader " << reader_id << ": Read record at index " << index << " = " << value << std::endl;
        file_out << "Reader " << reader_id << ": Read record at index " << index << " = " << value << std::endl;
...
void* Writer(void* arg) {
    ...
    while (true) {
        ...
        // протокол пакости райдера в консоль и в файл
        std::cout << "Writer " << writer_id << ": Updated record at index " << index << " from " << old_value << " to " << new_value << std::endl;
        file_out << "Writer " << writer_id << ": Updated record at index " << index << " from " << old_value << " to " << new_value << std::endl;
...
int main(int argc, char* argv[]) {
    ...
    // 0 - name of project, 1 - file_from, 2 - file_out
    if (argc == 3) {
        file_from.open(argv[1]);
        file_from >> readers_num >> writers_num;
        file_out.open(argv[2]);
        // 0 - pj, 1 - int_1, 2 - int_2, 3 - file_out
    } else if (argc == 4) {
